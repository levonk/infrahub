...
**Exception — public SSH keys in bootstrap scripts.** ADR-20260624001 §4 carves out a narrow exception: the operator-owned admin bootstrap **public** key (`lzkmbp2016-micro-oracle`) may be embedded as a default in `shared/scripts/bootstrap-{macos,linux}-manual.sh` and `bootstrap-windows-manual.ps1`. Public keys are non-secret by definition (they are published in `authorized_keys`, GitHub, DNS, etc.), and this key is operator-wide, not client-specific — it bootstraps `auser` on every client's hosts. The chicken-and-egg rationale: a fresh host cannot clone the client submodule to read its key path until it has been bootstrapped, so the bootstrap script must carry its own default. Client-specific keys (per-client CI deploy keys, per-client service accounts) still belong in the client submodule. See ADR-20260624001 §4 for the full rationale.
