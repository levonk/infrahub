...
# Base Kali+Nix Health Check Script
# This script checks if the Kali Linux + Nix environment is healthy and ready
...
# Function to check Nix environment
check_nix_env() {
    log "Checking Nix environment..."
    
    # Check if Nix is available
    if command -v nix >/dev/null 2>&1; then
        local nix_version=$(nix --version 2>/dev/null | head -1)
        log "✓ Nix available: $nix_version"
        
        # Check Nix profile
        if [ -d "/nix/var/nix/profiles/default" ]; then
            log "✓ Nix profile accessible"
            
            # Check if nixpkgs channel is available
            if [ -d "/nix/var/nix/profiles/default/channels/nixpkgs" ]; then
                log "✓ Nixpkgs channel available"
            else
                log "! Nixpkgs channel not available"
            fi
        else
            log "! Nix profile not accessible"
        fi
        
        return 0
    else
        log "✗ Nix not available"
        return 1
    fi
}
...
check_security_tools() {
    ...
    # Check for Nix-installed tools
    if command -v nix >/dev/null 2>&1; then
        local nix_tools=("cowsay")  # Basic test tool
        for tool in "${nix_tools[@]}"; do
            if nix profile list | grep -q "$tool"; then
                log "✓ $tool available via Nix"
            else
                log "! $tool not available via Nix"
            fi
        done
    fi
...
check_python_env() {
    ...
    if [ -d "$venv_dir" ] && [ -f "$venv_dir/bin/python" ]; then
        ...
        if "$venv_dir/bin/python" --version >/dev/null 2>&1; then
            ...
            local security_packages=("scapy" "pwntools" "cryptography")
            for package in "${security_packages[@]}"; do
                if "$venv_dir/bin/python" -c "import $package" 2>/dev/null; then
                    log "✓ Python package $package available"
                else
                    log "! Python package $package not available"
                fi
            done
...
check_user_scripts() {
    ...
    if [ -d "$scripts_dir" ]; then
        local scripts=("security-tools" "nix-security-tools" "init-workspace")
...
# Function to check Nix integration
check_nix_integration() {
    log "Checking Nix integration..."
    
    if command -v nix >/dev/null 2>&1; then
        # Test basic Nix functionality
        if nix --help >/dev/null 2>&1; then
            log "✓ Nix command functional"
        else
            log "✗ Nix command not functional"
            return 1
        fi
        
        # Test Nix profile functionality
        if nix profile list >/dev/null 2>&1; then
            log "✓ Nix profile functional"
        else
            log "! Nix profile not functional"
        fi
        
        # Test Nix search functionality
        if timeout 10 nix search nixpkgs cowsay >/dev/null 2>&1; then
            log "✓ Nix search functional"
        else
            log "! Nix search not functional (may be slow)"
        fi
        
        return 0
    else
        log "! Nix not available for integration test"
        return 1
    fi
}
...
main() {
    ...
    log "Starting Base Kali+Nix health check..."
    ...
    check_nix_integration || true
    
    if [ $exit_code -eq 0 ]; then
        log "Base Kali+Nix health check passed"
    else
        log "Base Kali+Nix health check failed"
    fi
