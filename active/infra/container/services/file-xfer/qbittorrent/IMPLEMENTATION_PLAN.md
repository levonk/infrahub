...
- **Output**:
  ...
  - `packages.x86_64-linux.docker-prod`: The minimal image (named `qbittorrent`).
  - `packages.x86_64-linux.docker-debug`: The debug image (named `qbittorrent-debug`).

### 2. Image Composition (`dockerTools.buildLayeredImage`)

- **Naming**: The `name` and `tag` attributes of the image will be set to the service slug `qbittorrent`.
