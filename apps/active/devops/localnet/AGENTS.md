...
Running from subdirectories causes multiple catastrophic failures:

**1. Network Conflicts (The "Pool Overlaps" Error)**

When you run `just base-up-internal` from `services/base/`, Docker Compose:
- Creates networks scoped to that subdirectory's project name
- Uses relative CIDR ranges that may overlap with existing networks
- Creates duplicate networks with the same name but different Docker project scopes

**Example Error:**
```
Network agent-network Error
Error response from daemon: invalid pool request: Pool overlaps with other one on this address space
```

**Root Cause:** The `docker-compose.base.yml` references `docker-compose.shared.yml` which defines networks. When run from `services/base/`, Docker resolves these networks relative to the base directory, creating:
- `base_agent-network` (scoped to base directory)
- But the compose file tries to create `agent-network` with the same CIDR
- This conflicts with any existing `agent-network` from previous runs in the parent directory
...
- Volume paths and build contexts break when resolved from wrong directory

**3. Environment Variable Issues**

- `.env` files are loaded relative to working directory
- Running from subdirectories causes missing or wrong environment variables
- Services fail to start or use incorrect configurations

### The Technical Explanation

Docker Compose uses the current working directory to:
1. **Determine project name** (defaults to directory name)
2. **Resolve relative paths** in compose files (`../../scripts/`, `../configs/`)
3. **Load environment files** (`.env`, `env.local`)
4. **Scope network names** (`localnet_agent-network` vs `base_agent-network`)

When you run from `services/base/`:
- Project name becomes `base` instead of `localnet`
- Networks get created as `base_agent-network`, `base_localnet-network`
- These use the same CIDR ranges defined in `docker-compose.shared.yml`
- If `localnet_agent-network` already exists with the same CIDR, Docker throws "Pool overlaps" error

### ❌ WRONG (will fail)

```bash
cd services/base
just base-up-internal  # ← FAILS - creates conflicting networks!
...
```bash
cd apps/active/devops/localnet
just base-up  # ← Uses correct paths, networks, and env vars
```

### Recovery From Network Conflicts

If you've already run from the wrong directory and hit network conflicts:

```bash
# 1. Clean up all containers and networks from wrong directory
cd services/base
docker compose down -v --remove-orphans

# 2. Remove conflicting networks manually
docker network rm base_agent-network base_localnet-network base_wireguard-network 2>/dev/null || true

# 3. Clean up any orphaned networks
docker network prune -f

# 4. Return to correct directory and run properly
cd ../..
just base-up
```
...
### Build Order vs Runtime Dependencies

**Critical distinction:** `depends_on` only controls **runtime** container startup order, **NOT** build order.

| Mechanism | Controls | Enforces Build Order? |
|-----------|----------|----------------------|
| `depends_on` | Runtime container startup | ❌ No |
| Profile sequencing (justfile) | Build execution order | ✅ Yes |

**Why this matters:**

Docker Compose builds all services in a profile **in parallel**. If service B's Dockerfile uses `FROM service-a:latest`, but service A hasn't finished building yet, the build fails with "pull access denied" because the image doesn't exist.

**The Solution - Profile-Based Build Sequencing:**
...
services:
  ...
  base-kalinix:
    ...
    profiles: [base02]  # Builds AFTER base-kali completes
    depends_on:
      base-kali:
        condition: service_started  # Runtime only - NOT build order!
```
...
**Profile Hierarchy for Base Services:**

| Profile | Services | Purpose |
|---------|----------|---------|
| `base01` | base-alpine, base-debian, base-kali, nix-sidecar | External base images |
| `base02` | base-kalinix, base-nix, base-debnix | Images depending on base01 |
| `base03` | base-sidecar, base-dev | Images depending on base02 |
