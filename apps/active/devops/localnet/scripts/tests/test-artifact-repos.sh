...
test_nexus_health() {
    ...
    if curl -sf "http://localhost:$NEXUS_WEB_PORT/service/rest/v1/status" > /dev/null; then
...
test_nexus_web_ui() {
    ...
    if curl -sf "http://localhost:$NEXUS_WEB_PORT/" | grep -q "Nexus"; then
...
test_nexus_repositories() {
    ...
    repos=$(curl -sf "http://localhost:$NEXUS_WEB_PORT/service/rest/v1/repositories" 2>/dev/null || echo "[]")
