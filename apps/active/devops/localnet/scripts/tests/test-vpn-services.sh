...
test_wireguard_direct_health() {
    ...
    if docker exec localnet-vpn-direct test -f /config/wg0.conf; then
...
test_wireguard_direct_interface() {
    ...
    if docker exec localnet-vpn-direct wg show 2>&1 | grep -q "interface: wg0"; then
...
test_wireguard_direct_port() {
    ...
    if docker exec localnet-vpn-direct ss -ulnp | grep -q ":$WG_DIRECT_PORT"; then
...
test_wireguard_transparent_health() {
    ...
    if docker exec localnet-vpn-transparent test -f /config/wg0.conf; then
...
test_wireguard_transparent_interface() {
    ...
    if docker exec localnet-vpn-transparent wg show 2>&1 | grep -q "interface: wg0"; then
...
test_wireguard_transparent_port() {
    ...
    if docker exec localnet-vpn-transparent ss -ulnp | grep -q ":$WG_TRANSPARENT_PORT"; then
...
test_transparent_gateway() {
    ...
    if docker ps --filter "name=localnet-proxy-transparent-gateway" --format "{{.Status}}" | grep -q "Up"; then
...
test_wireguard_peer_config() {
    ...
    peer_count=$(docker exec localnet-vpn-direct wg show wg0 peers 2>/dev/null | wc -l || echo "0")
...
test_wireguard_qr_codes() {
    ...
    if docker exec localnet-vpn-direct ls /config/peer* 2>/dev/null | grep -q "peer"; then
