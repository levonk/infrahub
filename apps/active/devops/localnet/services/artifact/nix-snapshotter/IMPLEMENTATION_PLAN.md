...
- **Output**:
  ...
  - `packages.x86_64-linux.docker-prod`: The minimal image (named `nix-snapshotter`).
  - `packages.x86_64-linux.docker-debug`: The debug image (named `nix-snapshotter-debug`).
