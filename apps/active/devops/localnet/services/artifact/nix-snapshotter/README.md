# {{ _service_name | title }} - Nix-based Docker Service
# Generated from boilerplate template

## Overview

Nix Snapshotter for K8s Nodes
...
1.  **Production Image (`nix-snapshotter:latest`)**:
...
2.  **Debug Image (`nix-snapshotter-debug:latest`)**:
...
docker run -it --rm nix-snapshotter-debug zsh
```
...
|----------|-------------|---------|
| `SERVICE_NAME` | Service identifier | `nix-snapshotter` |
| `SERVICE_PORT` | Port the service listens on | `8989` |
