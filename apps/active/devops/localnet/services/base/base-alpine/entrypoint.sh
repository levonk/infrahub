...
echo "🤖 Base Alpine: Starting entrypoint..."
...
echo "🤖 Base Alpine: Executing as $USERNAME: $@"
if [ -n "$SU_EXEC" ]; then
    exec "$SU_EXEC" "$USERNAME" "$@"
elif [ -n "$GOSU" ]; then
    exec "$GOSU" "$USERNAME" "$@"
else
    echo "❌ Base Alpine: Error: No su-exec or gosu found. Cannot drop privileges per ADR-20260106001."
    exit 1
fi
