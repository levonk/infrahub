...
LISTEN_PORT="${DNS_DNSCRYPT_STD_CONTAINER_PORT:-5053}"
...
if ! sed -i "s|{DNS_DNSCRYPT_STD_CONTAINER_PORT}|${LISTEN_PORT}|g" "$WORKING_CONFIG"; then
  echo "[ERROR] Failed to substitute port in config file" >&2
  exit 1
fi

# Substitute Tor proxy variables
if ! sed -i "s|\${PROXY_SOCKS5_TOR_IP}|${PROXY_SOCKS5_TOR_IP}|g" "$WORKING_CONFIG"; then
  echo "[ERROR] Failed to substitute Tor proxy IP in config file" >&2
  exit 1
fi

if ! sed -i "s|\${PROXY_SOCKS5_TOR_CONTAINER_PORT}|${PROXY_SOCKS5_TOR_CONTAINER_PORT}|g" "$WORKING_CONFIG"; then
  echo "[ERROR] Failed to substitute Tor proxy port in config file" >&2
  exit 1
fi

# Verify substitutions were successful
if grep -q "{DNS_DNSCRYPT_STD_CONTAINER_PORT}" "$WORKING_CONFIG"; then
  echo "[ERROR] Port placeholder still present in config after substitution" >&2
  exit 1
fi

if grep -q "\${PROXY_SOCKS5_TOR_IP}" "$WORKING_CONFIG"; then
  echo "[ERROR] Tor proxy IP placeholder still present in config after substitution" >&2
  exit 1
fi

if grep -q "\${PROXY_SOCKS5_TOR_CONTAINER_PORT}" "$WORKING_CONFIG"; then
  echo "[ERROR] Tor proxy port placeholder still present in config after substitution" >&2
  exit 1
fi
