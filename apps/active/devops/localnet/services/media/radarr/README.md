...
1.  **Production Image (`{{ _service_name_slug }}:latest`)**:
...
2.  **Debug Image (`{{ _service_name_slug }}-debug:latest`)**:
...
docker run -it --rm {{ _service_name_slug }}-debug zsh
```
...
|----------|-------------|---------|
| `SERVICE_NAME` | Service identifier | `{{ _service_name_slug }}` |
| `SERVICE_PORT` | Port the service listens on | `{{ _service_port }}` |
