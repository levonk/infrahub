# Netbird VPN - Nix-based Docker Service
...
1.  **Production Image (`vpn-netbird:latest`)**:
...
2.  **Debug Image (`vpn-netbird-debug:latest`)**:
...
docker run -it --rm vpn-netbird-debug zsh
```
...
|----------|-------------|---------|
| `SERVICE_NAME` | Service identifier | `vpn-netbird` |
| `SERVICE_PORT` | Port the service listens on | `8084` |
