...
OPENVPN_USER=$(devbox run -- ansible-vault view levonk/active/02-config/ansible/group_vars/infrahub-levonk-all.vault.yml --vault-password-file ~/.ansible/vault_password 2>/dev/null | grep vault_nordvpn_openvpn_user | cut -d: -f2 | tr -d ' ')
OPENVPN_PASS=$(devbox run -- ansible-vault view levonk/active/02-config/ansible/group_vars/infrahub-levonk-all.vault.yml --vault-password-file ~/.ansible/vault_password 2>/dev/null | grep vault_nordvpn_openvpn_pass | cut -d: -f2 | tr -d ' ')
