{
  description = "Twingate VPN Client";
  ...
  outputs = { self, nixpkgs }:
    let
      ...
      app = pkgs.stdenv.mkDerivation {
        ...
        buildPhase = ''
          echo "Building twingate-vpn..."
          # make build
        '';

        installPhase = ''
          mkdir -p $out/bin
          # cp bin/twingate-vpn $out/bin/

          # Placeholder for demonstration
          echo '#!${pkgs.runtimeShell}' > $out/bin/twingate-vpn
          echo 'echo "Starting twingate-vpn on port 0..."' >> $out/bin/twingate-vpn
          echo 'while true; do sleep 1; done' >> $out/bin/twingate-vpn
          chmod +x $out/bin/twingate-vpn
        '';
      };

      # Common configuration for both images
      commonConfig = {
        Cmd = [ "${app}/bin/twingate-vpn" ];
        ExposedPorts = {
          "0/tcp" = {};
        };
        Env = [
